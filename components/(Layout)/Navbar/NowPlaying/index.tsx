        ...
        <div className="flex-1 min-w-0 overflow-hidden">
          <div className="whitespace-nowrap inline-block animate-marquee">
            <div className="inline-block">
              <span className="font-semibold">{nowPlayingTrack?.item?.name}</span>
              <span className="mx-1.5 font-normal opacity-70">•</span>
              <span className="font-normal">{nowPlayingTrack?.item?.artists[0]?.name}</span>
              <span className="mx-1.5 font-normal opacity-70">•</span>
              <span className="font-light italic">{nowPlayingTrack?.item?.album?.name}</span>
          ...
          >
            ...
            <div className="absolute inset-0">
              ...
              <div className="absolute inset-0" style={{
                background: 'radial-gradient(circle at 30% 50%, rgba(61, 56, 245, 0.15) 0%, transparent 50%), radial-gradient(circle at 70% 50%, rgba(139, 135, 255, 0.12) 0%, transparent 50%)'
              }} />
            </div>

            {/* Content */}
            <div className="relative p-5">
              <div className="text-xs text-gray-600 dark:text-gray-400 mb-3 text-center">Now Playing</div>
              ...
              <div className="text-center mb-3">
                <h3 className="text-base font-medium mb-1 text-gray-900 dark:text-white">{nowPlayingTrack?.item?.name}</h3>
                <p className="text-xs mb-1 text-gray-700 dark:text-gray-300">{nowPlayingTrack?.item?.artists[0]?.name}</p>
                <p className="text-xs italic text-gray-600 dark:text-gray-400">{nowPlayingTrack?.item?.album?.name}</p>
              </div>

              {/* Genre */}
              {nowPlayingGenres.length > 0 && (
                <div className="text-center mb-3">
                  <p className="text-xs text-gray-600 dark:text-gray-400">{nowPlayingGenres.join(" • ")}</p>
                </div>
              )}
