...
export default function SiteCard({ site }: SiteCardProps) {
  return (
    <a
      ...
      className="group relative flex flex-col overflow-hidden bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-2xl hover:border-gray-300 dark:hover:border-white/20 transition-all duration-300 hover:scale-105 hover:shadow-lg"
    >
      {/* Featured Badge */}
      {site.featured && (
      ...
      <div className="flex-1 flex flex-col p-4">
        ...
        <div className="flex items-center justify-between pt-3 border-t border-gray-200 dark:border-white/10">
          <span className="text-xs leading-none text-gray-500 dark:text-gray-500 group-hover:text-[#3d38f5] dark:group-hover:text-[#8b87ff] transition-colors truncate flex-1">
            {new URL(site.url).hostname.replace("www.", "")}
          </span>
          <ExternalLink className="w-4 h-4 text-gray-400 dark:text-gray-600 group-hover:text-[#3d38f5] dark:group-hover:text-[#8b87ff] group-hover:translate-x-1 transition-all shrink-0 ml-2" />
        </div>
      </div>
    </a>
  );
