...
async function refreshAccessToken() {
  ...
  if (consecutiveFailures >= MAX_FAILURES) {
    if (now - lastTokenFetch < FAILURE_RESET_TIME) {
      ...
      return NextResponse.json(
        { 
          error: "Token service temporarily unavailable. Please check your Spotify credentials.",
          retryAfter: Math.ceil((FAILURE_RESET_TIME - (now - lastTokenFetch)) / 1000)
        },
        { status: 503 }
      );
    }
  ...
  // Rate limit: Don't fetch more than once per MIN_FETCH_INTERVAL
  ...
  try {
    ...
    if (err.message?.includes("400") || err.message?.includes("401")) {
      ...
      return NextResponse.json(
        { 
          error: "Invalid Spotify credentials. Please check your refresh token.",
          details: err.message
        },
        { status: 401 }
      );
