...
export default function AdminPage() {
  ...
  const cacheTags = [
    { tag: 'playlists', label: 'Playlists', description: 'User playlists list' },
    { tag: 'tracks', label: 'Tracks', description: 'Track metadata' },
    { tag: 'artist-genres', label: 'Artist Genres', description: 'Artist genre data' },
    { tag: 'curated-tracks', label: 'Curated Tracks', description: 'Homepage featured tracks' },
    { tag: 'recently-played', label: 'Recently Played', description: 'Last played track' },
    { tag: 'spotify-token', label: 'Access Token', description: 'Spotify auth token' },
  ];
    ...
    return (
      <div className="min-h-screen pt-20 sm:pt-24 pb-12 px-4 flex items-center justify-center">
        <div className="max-w-md w-full">
          ...
          <form onSubmit={handleLogin} className="bg-gray-100 dark:bg-white/5 rounded-lg border border-gray-200 dark:border-white/10 p-6">
            <div className="mb-4">
      ...
      <div className="max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {cacheTags.map(({ tag, label, description }) => (
              ...
              >
                ...
                >
                  <RefreshCw className={`w-4 h-4 ${refreshing === tag ? 'animate-spin' : ''}`} />
        ...
        <div className="bg-blue-50 dark:bg-blue-900/10 border border-blue-200 dark:border-blue-800/30 rounded-lg p-4">
          <p className="text-sm text-blue-800 dark:text-blue-300">
            <strong>Note:</strong> For detailed API logs and monitoring, check your Vercel dashboard under the "Logs" tab.
          </p>
        </div>
      </div>
    </div>
