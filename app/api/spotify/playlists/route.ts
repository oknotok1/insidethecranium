  ...
  try {
    ...
    if (includeGenres) {
      try {
          ...
          try {
            ...
            if (!tracksResponse.ok) {
              ...
              if (tracksResponse.status === 429) {
                logger.warn(
                  "Playlists API",
                  `Rate limited while fetching tracks, stopping genre fetch`,
                );
                break; // Stop processing more playlists
              }
              logger.warn(
                "Playlists API",
                `Failed to fetch tracks for ${playlist.name}: ${tracksResponse.status}`,
              );
              continue;
            }
            ...
            if (artistIds.size === 0) {
              logger.warn(
                "Playlists API",
                `No artists found for ${playlist.name}`,
              );
              continue;
            }
            ...
            if (artistsResponse.status === 429) {
              logger.warn(
                "Playlists API",
                `⚠ Rate limited while fetching artists, stopping genre fetch`,
              );
              break; // Stop processing more playlists
            }
            ...
            if (artistsResponse.ok) {
              ...
              logger.warn(
                "Playlists API",
                `Failed to fetch artists for ${playlist.name}: ${artistsResponse.status}`,
              );
            }
          } catch (error) {
            logger.warn(
              "Playlists API",
              `Failed to fetch genres for ${playlist.id}`,
            );
          }
        }
        ...
        logger.success(
          "Playlists API",
          `✓ Fetched genres for ${Object.keys(playlistGenres).length}/${playlistsToProcess.length} playlists (mostly cached)`,
        );
      } catch (error: any) {
        logger.error(
          "Playlists API",
