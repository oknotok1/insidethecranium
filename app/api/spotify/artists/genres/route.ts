...
export async function GET(request: NextRequest) {
  ...
  logger.log('Artists API', `Fetching ${artistIds?.split(',').length || 0} artists`);
  ...
  try {
        ...
        try {
          ...
          if (response.status === 429) {
            ...
            if (!result.shouldRetry) {
              logger.error('Artists API', `${result.message} [batch ${i + 1}/${batches.length}]`);
              responses.push({ artists: [] });
              break;
            }
            
            logger.warn('Artists API', `${result.message} [batch ${i + 1}/${batches.length}]`);
          ...
          if (!response.ok) {
            ...
            logger.error('Artists API', `Spotify API error: ${response.status} - ${errorText}`);
            responses.push({ artists: [] });
            break;
          }
          ...
          logger.error('Artists API', `Fetch error (attempt ${retryCount + 1}): ${error.message}`);
    ...
    logger.success('Artists API', `Returning ${artistsWithGenres.length} artists with genres`);
